...
class Matplotlibtest(Scene):
    def construct(self):
        ...
        dot = Dot()
        self.add(dot)
        plotbackground= ImageMobject("plot11.png").scale(3).move_to(DL).shift(UP)
        ...
        sin_curve.align_to(plotbackground, LEFT)


        plotbackground2= ImageMobject("plot11.png").scale(1.1).to_corner(UR)
        self.add(plotbackground2)
        self.add(sin_curve)
        self.wait()

        sin_curve2 = sin_curve.copy()
        sin_curve2.stretch_to_fit_width(plotbackground2.get_width())
        sin_curve2.stretch_to_fit_height(plotbackground2.get_height())
        sin_curve2.align_to(plotbackground2, DOWN)
        sin_curve2.align_to(plotbackground2, LEFT)
        self.play(Transform(sin_curve,sin_curve2))
        self.wait()

if __name__ == "__main__":
    module_name = os.path.basename(__file__)
    command_A = "manim -p  -m -c WHITE --video_dir ~/Downloads/  "
