...
class s3(Scene):
    def construct(self):
        ...
        offset_along= 0.5
        offset_connect =0.25
        U1 = p1+ senk*offset_senk -deltapnorm*offset_along
        U2 = p2+ senk*offset_senk +deltapnorm*offset_along
        V1 = p1 - senk*offset_senk -deltapnorm*offset_along
        V2 = p2 - senk*offset_senk+deltapnorm*offset_along
        s1 = p1-offset_connect*deltapnorm
        s2 = p2+offset_connect*deltapnorm
        LU = Line(U1, U2)
        LV = Line(V1, V2)
        self.add(LU, LV )
        Lp1s1 = DashedLine(p1, s1)
        # Ls1u1 = Line(s1,U1)
        # Ls1v1 = Line(s1,V1)
