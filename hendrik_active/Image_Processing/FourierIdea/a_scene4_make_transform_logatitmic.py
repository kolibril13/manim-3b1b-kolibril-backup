...
scene = "FirstTransformLOG"
class FirstTransformLOG(ThreeDScene):  # with real plane on the right
    def construct(self):
        ...
        k_disp.fill_k_space_updater(img_kamp,new_amp_max= True,logview=True,mushroom_heigth=1) #init wit new_amp_max ture
...
#        self.wait(2)
        ...
        def amp_tansformer(mob):
            ...
            k_math.apply_transformations(val,sigma=0.3,mode="lowpass")
            k_disp.set_magic_gauss(val,sigma=0.9, mode="lowpass")
            img_kamp, img_kph =k_math.get_amp_and_ph()
            k_disp.fill_k_space_updater(img_kamp,logview=True)
