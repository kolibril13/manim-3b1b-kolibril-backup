...
class Nearly_Empty_Fourier(ThreeDScene):
    ...
    def construct(self):
        ...
        def get_angle_phi(phi_deg):
            ...
            k_plane.set_y(0)
            k_plane.set_z(-0.01)
            k_plane.fill_k_space_updater(amp_ar, dots_lines=True)
...
class More_Filled_Fourier(ThreeDScene):
    ...
    def construct(self):
        ...
        def get_angle_phi(phi_deg):
            ...
            z=r * np.exp(1j * phi) #must be in radians here!!
            img_k_space[4,3]=z
            img_kamp = np.abs(img_k_space)
            img_kph = (np.angle(img_k_space, deg=True))
            ...
            k_plane.set_y(0)
            k_plane.set_z(-0.01)
            k_plane.fill_k_space_updater(img_kamp, dots_lines=True)
            k_plane.set_phase_flowers(img_kamp,img_kph)
            k_plane.scale_about_point(9 / pixels * k_plane_size, ORIGIN)
            k_plane.set_shade_in_3d(True)
            all.add(k_plane)
            # setup the real_out_plane
        ...
        self.play(flow_tracker.set_value, self.flower_value_end ,run_time=6, rate_func=linear)
