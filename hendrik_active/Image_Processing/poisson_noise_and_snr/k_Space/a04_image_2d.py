...
class ImgFourierSpace:
    ...
    def get_amp_and_ph(self):
        self.img_kamp = np.abs(self.img_k_space)
        self.img_kph = (np.angle(self.img_k_space, deg=True))
        return self.img_kamp, self.img_kph
...
class image2d_forier(Scene):
    def construct(self):
        pixels = 21  # setup the sizes
        img_forier_instance = ImgFourierSpace(preset="minimal",pixels=pixels)
        img_forier_instance.phase_shift(190)
        img_kamp,img_kph= img_forier_instance.get_amp_and_ph()
        my_plane = K_Space(pixel_len=pixels) ###
        my_plane.set_phase_flowers(img_kamp, img_kph)
        my_plane.move_to(ORIGIN)
        ...
        img_real=img_forier_instance.get_real_out().real
...
if __name__ == "__main__":
    ...
    command_B = module_name +" " +"image2d_forier"
